  ...
  return (
    <div className="p-6 h-full flex flex-col pt-safe-top overflow-y-auto no-scrollbar pb-24">
      <header className="mb-6 flex items-center gap-2">
        {view !== 'menu' && (
          <button onClick={() => setView('menu')} className="p-1 rounded-full bg-slate-100 hover:bg-slate-200 mr-2">
      ...
      {view === 'menu' && (
        <div className="space-y-6 animate-in fade-in">
          {/* Main Action */}
          <button onClick={() => setView('keywords')} className="w-full bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-left hover:shadow-md transition-all group">
      ...
      {view === 'keywords' && (
        <div className="animate-in slide-in-from-right-4 fade-in">
           <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 mb-6 space-y-4">
              ...
              <button onClick={handleGenerateKeywords} disabled={isGenerating || !niche} className="w-full bg-purple-600 hover:bg-purple-700 disabled:bg-purple-300 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2">
           ...
           {results.length > 0 && (
              <div className="space-y-3">
                 <div className="flex justify-between items-center bg-purple-50 p-3 rounded-xl mb-2">
                    ...
                    <button onClick={handleSaveAll} className="text-xs bg-purple-600 text-white px-3 py-1.5 rounded-lg font-bold shadow-sm active:scale-95 transition-transform flex items-center gap-1">
      ...
      {view === 'folder' && activeFolder && (
         <div className="animate-in slide-in-from-right-4 fade-in flex flex-col h-full">
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center mb-4">
               ...
               <button onClick={() => { onDeleteFolder(activeFolder.id); setView('menu'); }} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={20}/></button>
            </div>

            {/* TABS */}
            <div className="flex p-1 bg-slate-100 rounded-xl mb-4 shrink-0">
               <button onClick={() => setActiveTab('keywords')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'keywords' ? 'bg-white shadow-sm text-purple-600' : 'text-slate-400'}`}>Cuvinte Cheie ({activeFolder.keywords.length})</button>
               <button onClick={() => setActiveTab('plan')} className={`flex-1 py-2 text-xs font-bold rounded-lg transition-all ${activeTab === 'plan' ? 'bg-white shadow-sm text-purple-600' : 'text-slate-400'}`}>Calendar Editorial ({activeFolder.planItems?.length || 0})</button>
            </div>
                 ...
                 <div className="grid grid-cols-1 gap-2">
                    {activeFolder.keywords.map(k => (
                      <div key={k.id} className="bg-white p-3 rounded-xl border border-slate-100 flex justify-between items-center">
                         ...
                         <button onClick={() => handleDeleteKeywordFromFolder(k.id)} className="text-slate-300 hover:text-red-500"><X size={16}/></button>
                      </div>
                    ))}
                 </div>
                 {activeFolder.keywords.length > 0 && (
                   <button onClick={runStrategy} disabled={isStrategyLoading} className="w-full bg-slate-900 text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 mt-4">
              ...
              <div className="space-y-4 pb-20">
                 {(!activeFolder.planItems || activeFolder.planItems.length === 0) && (
                    <div className="text-center py-10">
                       ...
                       <button onClick={runStrategy} className="bg-purple-600 text-white px-4 py-2 rounded-lg font-bold text-sm">Generează Acum</button>
                    </div>
                 )}
