...
import { 
  ...
  Eye, EyeOff, Zap, CheckCircle, HelpCircle, FileCheck, Eraser, Activity, Wand2, Image as ImageIcon
} from 'lucide-react';
...
import { generateAnswerParagraph, generateTLDR, generateFAQSchema, cleanHTML, auditSEOContent, generateSEOMetadata, generateFeaturedImage } from '../../services/aiService';
...
export const ActiveProjectView: React.FC<ActiveProjectViewProps> = ({ 
  ...
  const handleGenerateImage = async () => {
    ...
    setGeneratedPrompt('');
    try {
        const result = await generateFeaturedImage(editorData.title, editorData.content, imgOptions);
        ...
        if (result.base64) {
            ...
            setEditorData(prev => ({ ...prev, featured_media: uploaded.id }));
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 font-sans">
      ...
      <div className="p-4 bg-white/80 backdrop-blur shadow-sm z-20 flex items-center gap-3 sticky top-0 pt-safe-top">
        <button onClick={() => { if(subView !== 'dashboard') setSubView('dashboard'); else onClose(); }} className="p-2 rounded-full hover:bg-slate-100 transition-colors">
        ...
        {project.status === 'connected' && (
          <button onClick={() => onSync(project)} className={`p-2 rounded-full bg-blue-50 text-blue-600 ${loadingProjectId === project.id ? 'animate-spin' : ''}`}>
            <RefreshCw size={20} />
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-32 no-scrollbar">
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-slate-100 mb-6 overflow-hidden transition-all duration-300">
            {!isEditingConfig ? (
              <div className="p-4 flex items-center justify-between group cursor-pointer hover:bg-slate-50" onClick={() => setIsEditingConfig(true)}>
                ...
                <button className="p-2 text-slate-400 hover:text-blue-600 bg-transparent rounded-full"><Edit2 size={18} /></button>
              </div>
            ) : (
              <div className="p-5 animate-in fade-in slide-in-from-top-2">
                 <div className="flex justify-between mb-4">
                    ...
                    <button onClick={() => setIsEditingConfig(false)}><X size={20} className="text-slate-400" /></button>
                 </div>
                 ...
                 <div className="space-y-4">
                    ...
                    <button onClick={() => onConnect(project)} disabled={loadingProjectId === project.id} className="w-full bg-slate-900 text-white font-bold py-3.5 rounded-xl flex justify-center items-center gap-2 shadow-lg active:scale-95 transition-transform">
        ...
        {project.status === 'connected' && subView === 'dashboard' && (
           <div className="grid grid-cols-1 gap-4 animate-in fade-in">
              ...
              <button onClick={() => setSubView('list')} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-left hover:shadow-md transition-all group">
              ...
              <button onClick={() => handleCreateNew} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 text-left hover:shadow-md transition-all group">
        ...
        {project.status === 'connected' && subView === 'list' && (
           <div className="animate-in slide-in-from-right-4 fade-in">
              <div className="flex items-center justify-between mb-4">
                 ...
                 <button onClick={handleCreateNew} className="bg-slate-900 text-white p-2 rounded-lg shadow-md active:scale-95"><Plus size={18}/></button>
              </div>
              <div className="flex p-1 bg-slate-100 rounded-xl mb-4">
                {(['all', 'publish', 'draft'] as const).map(s => (
                  <button key={s} onClick={() => setFilterStatus(s)} className={`flex-1 py-1.5 text-xs font-bold rounded-lg capitalize transition-all ${filterStatus === s ? 'bg-white shadow-sm text-slate-800' : 'text-slate-400'}`}>{s}</button>
                ))}
              </div>
           ...
           <div className="animate-in slide-in-from-right-4 fade-in flex flex-col pb-6">
              ...
              <div className="flex items-center justify-between mb-4">
                 ...
                 <div className="flex gap-2">
                    {editorData.id && <button onClick={handleDuplicatePost} className="p-2 text-indigo-500 hover:bg-indigo-50 rounded-lg"><Copy size={20}/></button>}
                    {editorData.id && <button onClick={handleDeletePost} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={20}/></button>}
                 </div>
              </div>

              {/* EDITOR TABS */}
              <div className="flex p-1 bg-slate-100 rounded-xl mb-4">
                 <button onClick={() => setEditorTab('edit')} className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${editorTab === 'edit' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}><Edit2 size={14}/> Edit</button>
                 <button onClick={() => setEditorTab('preview')} className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${editorTab === 'preview' ? 'bg-white shadow-sm text-blue-600' : 'text-slate-400'}`}><Eye size={14}/> Preview</button>
                 <button onClick={() => setEditorTab('aeo')} className={`flex-1 py-2 text-xs font-bold rounded-lg flex items-center justify-center gap-2 transition-all ${editorTab === 'aeo' ? 'bg-white shadow-sm text-purple-600' : 'text-slate-400'}`}><Zap size={14}/> AEO Studio</button>
              </div>
                ...
                <div className="space-y-4 mb-20 animate-in fade-in">
                   <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                         <button onClick={handleInjectSnippet} disabled={aeoProcessing} className="bg-white p-3 rounded-xl text-left border border-purple-100 hover:border-purple-300 shadow-sm active:scale-95 transition-all">
                         ...
                         <button onClick={handleInjectTLDR} disabled={aeoProcessing} className="bg-white p-3 rounded-xl text-left border border-purple-100 hover:border-purple-300 shadow-sm active:scale-95 transition-all">
                         ...
                         <button onClick={handleGenerateFAQ} disabled={aeoProcessing} className="bg-white p-3 rounded-xl text-left border border-purple-100 hover:border-purple-300 shadow-sm active:scale-95 transition-all">
                         ...
                         <button onClick={handleCleanHTML} disabled={aeoProcessing} className="bg-white p-3 rounded-xl text-left border border-purple-100 hover:border-purple-300 shadow-sm active:scale-95 transition-all">
                   ...
                   <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                      ...
                      <div className="space-y-3">
                         <div className="grid grid-cols-2 gap-2">
                             <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Stil Imagine</label>
                                <select value={imgOptions.style} onChange={e => setImgOptions({...imgOptions, style: e.target.value as any})} className="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none">
                             ...
                             <div>
                                <label className="text-[10px] font-bold text-slate-400 uppercase">Text Overlay</label>
                                <input value={imgOptions.textOverlay || ''} onChange={e => setImgOptions({...imgOptions, textOverlay: e.target.value})} className="w-full text-xs border border-slate-200 rounded-lg p-2 outline-none" placeholder="Optional" />
                             </div>
                         </div>
                         <button onClick={handleGenerateImage} disabled={aeoProcessing} className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white font-bold py-2 rounded-xl text-xs flex items-center justify-center gap-2 shadow-md">
                         ...
                         {generatedPrompt && (
                             <div className="bg-slate-50 p-2 rounded-lg border border-slate-100">
                                ...
                                <button onClick={() => navigator.clipboard.writeText(generatedPrompt)} className="text-xs text-blue-600 font-bold flex items-center gap-1"><Copy size={12}/> Copiază</button>
                             </div>
                         )}
                      </div>
                   </div>
                   ...
                   <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100">
                      <div className="flex justify-between items-center mb-4">
                         ...
                         <button onClick={handleAuditSEO} disabled={aeoProcessing} className="bg-slate-900 text-white px-3 py-1.5 rounded-lg text-xs font-bold disabled:opacity-50 flex items-center gap-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs">
                               <div className={`p-2 rounded border flex flex-col justify-between gap-2 ${aeoAudit.checklist.hasAnswerParagraph ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                                  ...
                                  {aeoAudit.checklist.hasAnswerParagraph ? <CheckCircle size={16}/> : (
                                    <button onClick={(e) => { e.stopPropagation(); handleInjectSnippet(); }} disabled={aeoProcessing} className="bg-white/50 hover:bg-white text-red-700 border border-red-200 px-2 py-1 rounded text-[10px] font-bold w-full text-center">Fix It</button>
                                  )}
                               </div>
                               <div className={`p-2 rounded border flex flex-col justify-between gap-2 ${aeoAudit.checklist.hasTLDR ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                                  ...
                                  {aeoAudit.checklist.hasTLDR ? <CheckCircle size={16}/> : (
                                    <button onClick={(e) => { e.stopPropagation(); handleInjectTLDR(); }} disabled={aeoProcessing} className="bg-white/50 hover:bg-white text-red-700 border border-red-200 px-2 py-1 rounded text-[10px] font-bold w-full text-center">Fix It</button>
                                  )}
                               </div>
                               <div className={`p-2 rounded border flex flex-col justify-between gap-2 ${aeoAudit.checklist.hasFAQ ? 'bg-green-50 border-green-200 text-green-700' : 'bg-red-50 border-red-200 text-red-700'}`}>
                                  ...
                                  {aeoAudit.checklist.hasFAQ ? <CheckCircle size={16}/> : (
                                    <button onClick={(e) => { e.stopPropagation(); handleGenerateFAQ(); }} disabled={aeoProcessing} className="bg-white/50 hover:bg-white text-red-700 border border-red-200 px-2 py-1 rounded text-[10px] font-bold w-full text-center">Fix It</button>
                                  )}
                               </div>
              ...
              {editorTab === 'edit' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-4">
                   <button onClick={() => setIsTaxonomyOpen(!isTaxonomyOpen)} className="w-full p-4 flex justify-between bg-slate-50"><div className="flex gap-2 font-bold text-sm text-slate-700"><Tags size={16}/> Categorii</div>{isTaxonomyOpen?<ChevronUp size={16}/>:<ChevronDown size={16}/>}</button>
                   {isTaxonomyOpen && (
                      <div className="p-5 space-y-4 border-t border-slate-100">
                         <div className="max-h-32 overflow-y-auto bg-slate-50 p-2 rounded border border-slate-200">{project.categories.map(c => (
                            <label key={c.id} className="flex gap-2 p-1.5 hover:bg-slate-100 rounded cursor-pointer"><input type="checkbox" checked={editorData.categories.includes(c.id)} onChange={() => setEditorData(p => ({...p, categories: p.categories.includes(c.id) ? p.categories.filter(x => x!==c.id) : [...p.categories, c.id]}))} className="accent-blue-600"/> <span className="text-sm">{c.name}</span></label>
                         ))}</div>
                         <div className="flex gap-2"><input value={newCatName} onChange={e=>setNewCatName(e.target.value)} className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-blue-100" placeholder="Categorie nouă" /><button onClick={() => handleCreateTax('cat')} className="bg-slate-800 text-white px-3 rounded-lg text-xs font-bold">+</button></div>
                      </div>
                   )}
                </div>
              )}
              ...
              {editorTab === 'edit' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-20">
                   <button onClick={() => setIsSeoOpen(!isSeoOpen)} className="w-full p-4 flex justify-between bg-slate-50"><div className="flex gap-2 font-bold text-sm text-slate-700"><Search size={16}/> SEO</div>{isSeoOpen?<ChevronUp size={16}/>:<ChevronDown size={16}/>}</button>
                   {isSeoOpen && (
                      <div className="p-5 space-y-3 border-t border-slate-100">
                         ...
                         <div className="flex gap-2">
                             ...
                             <button onClick={handleAutoGenerateSEO} disabled={aeoProcessing} className="bg-purple-100 text-purple-600 p-2 rounded-lg" title="Generează Auto cu AI"><Wand2 size={16}/></button>
                         </div>
